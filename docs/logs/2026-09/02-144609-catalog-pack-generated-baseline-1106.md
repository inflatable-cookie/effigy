...
## Exact-Head Review Repair (reviewed head `85b905bfc4c7e49fd95e8e82e77ce6264504d7dc`)

Exact-head review found one blocking `validation-gap`: on the reviewed head,
repository-owned `effigy doctor --json` returned `ok: false` with
`scan.generated-in-src` at severity `error` for
`crates/effigy-catalog/src/pack/baseline.rs`
(content-marker/generated-marker, 32.7 KB). The hand-authored verifier module
tripped the repository-owned generated-source scan: its module header
contained the marker substring `generated by` inside the prose phrase
"a generated byte-for-byte copy" (the scan samples the first 24 lines and
matches marker substrings case-insensitively; `generated b`… starts with
`generated by`). Neither CI nor `effigy qa` exercises this doctor invariant,
so the closeout overstated unchanged repository health.

Repair on the same branch, without weakening the scanner:

- `baseline.rs` header reworded to "a byte-for-byte generated copy" — same
  meaning, no marker substring anywhere in the file.
- Generated markers preserved on the snapshot/lock: the committed
  `catalog-pack.lock.toml` keeps its `GENERATED FILE — DO NOT HAND-EDIT.`
  header, and the byte-exact snapshot itself intentionally carries no extra
  marker file (that would change content identity).
- Recurrence proof added inside the verifier's own tests:
  `verifier_source_header_avoids_generated_marker_tokens` mirrors the scan
  marker tokens (`@generated`, `auto-generated`, `autogenerated`,
  `code generated`, `do not edit`, `generated by`) and the 24-line lowercase
  header window against the real file, and
  `committed_lock_keeps_its_generated_marker_header` asserts the lock marker
  header stays.
- Repository-owned assertion on the repaired head: `effigy doctor --json`
  returns `ok: true` with zero `error` findings (14 pre-existing
  warning-level `scan.god-files` findings remain, as before this lane).
- Focused baseline suite: 15 `pack::baseline` tests plus pack neighbors — 24
  passed, 0 failed; full `effigy-catalog` suite 165 lib + 57 integration
  green; `cargo clippy -p effigy-catalog --all-targets -- -D warnings` clean;
  `cargo fmt --all -- --check` clean; `effigy docs check links` passed;
