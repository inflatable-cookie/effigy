...
title: "Effigy g10.012 — Doctor subprocess deadlines"
...
The release-readiness audit found the defect now owned by [g10.012](../roadmaps/g10/012-doctor-subprocess-deadlines.md). Implement that ready Northstar task through the normal worker PR loop. The task card is the dispatch manifest and acceptance authority.

## Why It Matters

Overall structural/deep doctor deadlines bound Cargo metadata and Git identity children and clean their process trees. The operator requires this before the 0.13.0 release candidate can be ready.

## Current State

- Base: pushed integration `main`; Queue creates the isolated worker workspace.
- Governing and likely owner paths: Architecture 029 and contract 047; doctor, scan inventory, dependency subprocess, and runner doctor ports.
...
Real CLI blocked-child fixtures for structural and deep paths, shared overall budget, nonzero partial report, no survivor child or partial cache publication. Read the card and governing references before implementation. Add a user-facing `[Unreleased]` changelog entry. Report solvable friction in `PAPERCUTS.md` without diverting this task.
