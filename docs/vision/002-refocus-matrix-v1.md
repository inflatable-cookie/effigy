...
3. Patch log template guidance (logs index + authoring docs) with target-delta requirement.
4. Keep implementation checklists and closeout evidence in log history, not the vision strategy folder.

## 5. Acceptance Signals for Batch 1

1. Reviewers can identify a roadmap/guide's target movement in under 30 seconds.
2. New logs include explicit movement against one or more vision tags.
