...
- [x] Keep process lifecycle deterministic (start order, shutdown order, signal handling, exit propagation).
...
- [x] No replacement of normal non-TUI command output paths.
...
- Optional shell tab can be enabled by task config.
...
- [x] Implement lifecycle policy: startup ordering, cancellation, and graceful shutdown.
...
- [x] Add keymap help overlay and deterministic key handling.

### Phase 4.4 - Dev Ergonomics and Safety
- [x] Add restart controls (focused process restart and stop controls for focused process).
- [x] Add bounded scrollback ring buffer per process.
- [x] Add exit summary and non-zero propagation rules.
...
- [x] Graceful shutdown works from one key chord and cleans child processes reliably.
