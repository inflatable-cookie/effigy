# Roadmap g01

`g01` is the original Effigy implementation and consolidation generation.

Historical delivered milestones:

- [`001-effigy-foundation.md`](./001-effigy-foundation.md)
- [`002-deferral-fallback-system.md`](./002-deferral-fallback-system.md)
- [`003-coloured-output-and-widgets.md`](./003-coloured-output-and-widgets.md)
- [`004-dev-process-manager-tui.md`](./004-dev-process-manager-tui.md)
- [`005-unified-testing-orchestration.md`](./005-unified-testing-orchestration.md)
- [`006-tui-terminal-emulation.md`](./006-tui-terminal-emulation.md)
- [`007-test-runner-selection-and-environment-policy.md`](./007-test-runner-selection-and-environment-policy.md)
- [`008-universal-json-command-coverage.md`](./008-universal-json-command-coverage.md)
- [`009-doctor-health-consolidation.md`](./009-doctor-health-consolidation.md)
- [`010-dag-lock-policy-baseline.md`](./010-dag-lock-policy-baseline.md)
- [`011-watch-init-migrate-foundation.md`](./011-watch-init-migrate-foundation.md)
- [`012-codebase-consolidation-and-health.md`](./012-codebase-consolidation-and-health.md)
- [`013-effigy-northstar-doctrine-alignment.md`](./013-effigy-northstar-doctrine-alignment.md)
- [`014-attention-marker-scan-and-doctor-integration.md`](./014-attention-marker-scan-and-doctor-integration.md)
- [`015-effigy-self-hosting-and-agent-first-adoption.md`](./015-effigy-self-hosting-and-agent-first-adoption.md)

Recently completed milestones:

- [`016-duplicate-blocks-scan-and-doctor-integration.md`](./016-duplicate-blocks-scan-and-doctor-integration.md) (complete)
- [`017-comment-ratio-scan-and-doctor-integration.md`](./017-comment-ratio-scan-and-doctor-integration.md) (complete)
- [`018-generated-in-src-scan-and-doctor-integration.md`](./018-generated-in-src-scan-and-doctor-integration.md) (complete)
- [`019-stale-suppressions-scan-and-doctor-integration.md`](./019-stale-suppressions-scan-and-doctor-integration.md) (complete)
- [`020-research-phase-1-core-execution.md`](./020-research-phase-1-core-execution.md) (complete)
- [`021-research-phase-2-developer-experience.md`](./021-research-phase-2-developer-experience.md) (complete)
- [`022-research-phase-3-scale-and-integration.md`](./022-research-phase-3-scale-and-integration.md) (complete)
- [`023-builtin-test-suite-lifecycle-and-env.md`](./023-builtin-test-suite-lifecycle-and-env.md) (complete)
- [`024-release-pipeline-validation-and-consumer-ci.md`](./024-release-pipeline-validation-and-consumer-ci.md) (complete)
- [`026-changelog-library-and-northstar-profile.md`](./026-changelog-library-and-northstar-profile.md) (complete)
- [`027-release-orchestration-system.md`](./027-release-orchestration-system.md) (complete)
- [`029-northstar-effigy-consumer-adoption-kit.md`](./029-northstar-effigy-consumer-adoption-kit.md) (complete)

Research carry-forward:
- [`../../research/carry-forward-intake.md`](../../research/carry-forward-intake.md)

Release and distribution:
- no open `g01` release/distribution milestones remain

Rules:

- Keep `Generation: g01` markers intact in imported numbered files.
- `g01` is now closed as the implementation and consolidation generation.
- New roadmap work starts in the current generation.

## Next Task

Continue in the current generation front door:
[`../README.md`](../README.md).


## Historical language boundary

- New roadmaps and actively maintained roadmap updates must use roadmap IDs and batch language.
- Older imported roadmap bodies may retain internal `Phase X.Y` execution headings as historical record.
- Leave those historical headings alone unless that roadmap is reopened for active work, then normalize it in the same batch.


Batch cards live in `g01/batch-cards/` when strict posture uses them.
