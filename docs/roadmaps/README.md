...
- `g02/003-demo-harness-model-and-runner-contract.md` defines first-class demo proof and the runner/browser semantics that should sit inside Effigy; registry loading, inspection, `demo run`, lifecycle control, browser-state/query polish, repo-self-hosted proof demos, browser list/detail foundation, browser artifact-opening affordances, bounded browser live log visibility, in-browser query controls, and detail-pane navigation are now shipped, and the next bounded slice is browser metadata-query parity rather than wider runtime work.
...
- `docs/specs/batch-cards/030-implement-demo-browser-metadata-query-parity.md`
...
Execute the active ready card for `g02.003` to implement bounded browser
metadata-query parity now that detail-pane navigation is shipped.
