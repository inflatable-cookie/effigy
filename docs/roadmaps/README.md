...
- `g02/003-demo-harness-model-and-runner-contract.md` defines first-class demo proof and the runner/browser semantics that should sit inside Effigy; registry loading, inspection, `demo run`, lifecycle control, browser-state/query polish, repo-self-hosted proof demos, browser list/detail foundation, browser artifact-opening affordances, and bounded browser live log visibility are now shipped, and the next bounded slice is in-browser query controls on top of the existing demo-list contract.
...
- `docs/specs/batch-cards/026-implement-demo-browser-query-controls.md`
...
Execute the active ready card for `g02.003` to implement bounded browser query
controls now that live log visibility is shipped.
