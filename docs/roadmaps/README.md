...
- `g02/003-demo-harness-model-and-runner-contract.md` defines first-class demo proof and the runner/browser semantics that should sit inside Effigy; registry loading, inspection, `demo run`, lifecycle control, browser-state/query polish, repo-self-hosted proof demos, browser list/detail foundation, and browser artifact-opening affordances are now shipped, and the next bounded slice is deciding the post-artifact browser follow-up.
...
- `docs/specs/batch-cards/023-decide-demo-post-artifact-follow-up-boundary.md`
...
Execute the active ready card for `g02.003` to decide whether live log
visibility is the next honest browser follow-up now that artifact-opening is
shipped.
