...
- `g02/003-demo-harness-model-and-runner-contract.md` defines first-class demo proof and the runner/browser semantics that should sit inside Effigy; registry loading, inspection, `demo run`, and the first honest lifecycle-control slice are now shipped, and the next bounded slice is browser-state/query polish.
...
- `docs/specs/batch-cards/018-implement-demo-browser-state-and-query-polish.md`
...
Execute the active ready card for `g02.003` to implement browser-facing
state/query polish now that active-attempt, stop, and rerun control are
shipped.
