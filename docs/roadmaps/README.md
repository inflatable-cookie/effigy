...
- `g02/003-demo-harness-model-and-runner-contract.md` defines first-class demo proof and the runner/browser semantics that should sit inside Effigy; registry loading, inspection, `demo run`, lifecycle control, browser-state/query polish, repo-self-hosted proof demos, browser list/detail foundation, and browser artifact-opening affordances are now shipped, and the next bounded slice is implementing live log visibility in the browser.
...
- `docs/specs/batch-cards/024-implement-demo-browser-live-log-visibility.md`
...
Execute the active ready card for `g02.003` to implement bounded live log
visibility in the demo browser now that artifact-opening is shipped.
