...
| `effigy docs` | Run reusable docs QA checks such as link validation, JSON example validation, markdown index consistency checks, and log-index entry insertion | `check-links`, `check-json-examples`, `check-index`, `add-log-index`, `--repo`, `--file`, `--section`, `--min-blocks`, `--require`, `--require-block`, `--dir`, `--index`, `--json` | `effigy.docs.link-check.v1`, `effigy.docs.json-examples.v1`, `effigy.docs.index-check.v1`, `effigy.docs.add-log-index.v1` | `029-docs-qa-checklist-and-validation.md` |
