...
| `effigy deploy` | Derive a provider-neutral production deployment model and export the first bounded provider files from the effective manifest and bundle | `model`, `export render`, `--repo`, `--path`, `--plan`, `--json` | `deploy.model.v1`, `effigy.deploy.export.v1` | `002-production-deployment-model.md` |
