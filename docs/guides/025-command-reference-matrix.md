...
- Need to clone or update a repo and run its declared bring-up path: use
  `effigy bootstrap`.
- Need proof/demo discovery or inspection: use `effigy demo`.
...
| `effigy demo` | Discover repo-owned proof demos and inspect one demo's proof metadata, coverage, sources, and latest known receipt/artifact state | `list`, `inspect`, `--repo`, `--json` | `effigy.demo.list.v1`, `effigy.demo.inspect.v1` | `022-manifest-cookbook.md` |
...
- `demo inspect` reads declared receipt/artifact references and normalizes the
  latest known proof state; it does not execute the demo yet.
