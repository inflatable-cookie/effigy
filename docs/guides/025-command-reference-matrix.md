...
| `effigy graph` | Build, query, and keep the local code graph fresh for file, symbol, edge, impact, bounded context-pack, and watcher-driven agent lookup | `index`, `status`, `search`, `files`, `node`, `callers`, `callees`, `impact`, `context`, `watch`, `--repo`, `--json`, `--limit`, `--max-files`, `--max-bytes`, `--language`, `--path`, `--debounce-ms` | `effigy.graph.index.v1`, `effigy.graph.status.v1`, `effigy.graph.search.v1`, `effigy.graph.files.v1`, `effigy.graph.node.v1`, `effigy.graph.callers.v1`, `effigy.graph.callees.v1`, `effigy.graph.impact.v1`, `effigy.graph.context.v1`, `effigy.graph.watch.event.v1` | `076-code-graph-and-agent-workflows.md` |
...
| `effigy init` | Scaffold baseline `effigy.toml` (and any extra starter files such as root `README.md`) from a named starter (e.g. `minimal`, `northstar`) or list available starters | `<name>`, `--list`, `--dry-run`, `--force`, `--json` | `effigy.init.v1` | `019-watch-init-migrate-foundation.md` |
