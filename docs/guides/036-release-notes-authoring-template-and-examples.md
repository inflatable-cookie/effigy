...
- Keep `effigy repo contracts check-json --fast --print-selected=json` for local preflight.

## Validation
- command: `effigy repo contracts check-json --full --print-selected=json`
...
- Validation check `effigy repo docs check workflow-paths` intentionally excludes `docs/logs/` to preserve historical evidence fidelity.
