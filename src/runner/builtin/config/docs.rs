...
mod tests {
    use super::{
        package_manager_lines, tasks_canonical_lines, test_section_lines, ConfigDocProfile,
    };
    ...
    fn tasks_cache_comment_profile_contract_is_stable() {
        ...
        assert!(reference
            .contains(&"# Phase-1 task cache contract: explicit opt-in declarations only."));
        assert!(schema
            .contains(&"# Phase-1 cache contract: explicit opt-in only, no implicit discovery."));
