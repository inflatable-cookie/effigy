...
fn run_secrets_change_passphrase(
    ...
    let vault_path = resolve_vault_read_path(repo_root, Some(secrets))?;
...
/// Vault path for commands that read (or read-modify-write) an existing vault.
///
/// In a linked worktree with no vault of its own this resolves to the primary
/// checkout's vault, so `get`/`set`/`export` operate on the one machine-local
...
fn inspect_vault_doctor_state(
    ...
    let Ok(vault_path) = resolve_vault_read_path(repo_root, Some(secrets)) else {
