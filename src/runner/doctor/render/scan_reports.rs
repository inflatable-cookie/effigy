...
use super::super::contracts::check_id;
use super::super::report::DoctorSeverity;
...
pub(super) fn sync_scan_detail_reports(
    ...
    for stale_file_name in known_scan_report_file_names() {
        ...
        if stale_path.exists() {
            fs::remove_file(&stale_path)
                .map_err(|error| RunnerError::task_invocation_failed_write(&stale_path, error))?;
        }
    }
    ...
    fs::create_dir_all(&report_dir)
        .map_err(|error| RunnerError::task_invocation_failed_write(&report_dir, error))?;
    ...
    for (section, file_name) in active_scan_sections {
        ...
        fs::write(&report_path, render_scan_detail_report(section))
            .map_err(|error| RunnerError::task_invocation_failed_write(&report_path, error))?;
        paths.insert(
