...
use effigy_tui::demo_browser::{
    ...
    browser_header_lines, browser_list_summary_lines, browser_live_terminal_env, browser_vt_lines,
    first_demo_id, history_detail_render, overview_detail_render, query_summary,
    read_recent_log_lines, render_browser_demo_row, resolve_repo_relative_path, row_contains_demo,
    sanitize_live_terminal_bytes, selected_list_highlight_style, selected_list_highlight_symbol,
    take_complete_terminal_bytes, terminal_detail_render, BrowserFocus, DetailSelectableItem,
...
use super::{
    DemoArgs, DemoBrowserApp, DemoHistoryAttemptHistoryPayload, DemoLatestAttempt, DemoListGroupBy,
    DemoSummary,
};
...
fn browser_detail_lines_use_compact_sections() {
    ...
    assert!(!rendered.contains("Browser Proof Report"));
    assert!(
        rendered.find("tags: self-hosted, proof") < rendered.find("covers: effigy.demo.browser")
    );
...
fn browser_artifacts_tab_renders_artifact_entries() {
    ...
    detail.tags = vec!["self-hosted".to_owned()];
    detail.covers = vec!["effigy.demo.browser".to_owned()];
    let rendered = artifacts_detail_render(&detail, Some(DetailSelectableItem::Artifact(1)), true)
        .lines
        .into_iter()
        .map(|line: Line<'static>| line.to_string())
        .collect::<Vec<_>>()
