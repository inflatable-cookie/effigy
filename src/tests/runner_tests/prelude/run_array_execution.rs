...
use super::errors::assert_task_command_failure_code;
use super::harness::{temp_workspace, write_manifest};
use super::managed::write_catalog_manifest_with_alias;
use super::run_array::{run_validate_err, run_validate_ok};
use super::runtime::{fs, Duration, EnvGuard, Instant, Path, PathBuf};

pub(in crate::runner::tests) struct RunArrayRuntimeFlowCase {
    ...
    pub(in crate::runner::tests) dag_max_parallel: Option<u32>,
    pub(in crate::runner::tests) expected_marker: &'static [&'static str],
    pub(in crate::runner::tests) start_markers: &'static [&'static str],
    pub(in crate::runner::tests) end_markers: &'static [&'static str],
    pub(in crate::runner::tests) min_elapsed_ms: Option<u64>,
    pub(in crate::runner::tests) setup: fn(&Path, &Path),
}

pub(in crate::runner::tests) struct RunArrayRuntimeErrorCase {
    pub(in crate::runner::tests) workspace: &'static str,
    pub(in crate::runner::tests) args: &'static [&'static str],
    pub(in crate::runner::tests) marker_rel: Option<&'static str>,
    pub(in crate::runner::tests) expected_marker: &'static [&'static str],
    pub(in crate::runner::tests) expected_code: Option<Option<i32>>,
    pub(in crate::runner::tests) setup: fn(&Path, Option<&Path>),
}
...
pub(in crate::runner::tests) fn read_marker_text(marker: &Path) -> String {
    fs::read_to_string(marker).expect("read marker")
}

pub(in crate::runner::tests) fn read_marker_lines(marker: &Path) -> Vec<String> {
...
pub(in crate::runner::tests) fn assert_started_before_any_end(
    lines: &[String],
    start_markers: &[&str],
    end_markers: &[&str],
) {
...
pub(in crate::runner::tests) fn assert_run_array_runtime_flow_case_table(
...
pub(in crate::runner::tests) fn assert_run_array_runtime_error_case_table(
...
pub(in crate::runner::tests) fn expected_cargo_paths(root: &Path) -> String {
...
pub(in crate::runner::tests) fn dag_max_parallel_env(parallelism: u32) -> EnvGuard {
    EnvGuard::set_many(&[("EFFIGY_DAG_MAX_PARALLEL", Some(parallelism.to_string()))])
}

pub(in crate::runner::tests) fn write_env_files(root: &Path, entries: &[(&str, &str)]) {
    for (relative, body) in entries {
        fs::write(root.join(relative), body).expect("write env file");
    }
}

pub(in crate::runner::tests) fn write_catalog_api_single_env_capture_manifest(
    root: &Path,
    catalog_dir: &str,
    alias: &str,
    env_step_expr: &str,
...
pub(in crate::runner::tests) fn write_catalog_api_unreachable_manifest(
    root: &Path,
    catalog_dir: &str,
    alias: &str,
    env_step_expr: &str,
...
pub(in crate::runner::tests) fn write_root_api_single_env_capture_manifest(
    root: &Path,
    marker: &Path,
    env_prelude: Option<&str>,
    env_step_expr: &str,
...
pub(in crate::runner::tests) fn write_root_api_dual_env_capture_manifest(
    root: &Path,
    marker: &Path,
    env_prelude: Option<&str>,
    env_step_exprs: &[&str],
...
pub(in crate::runner::tests) fn write_task_api_env_capture_manifest(
    root: &Path,
    marker: &Path,
    task_env_file_expr: Option<&str>,
    step_env_file_expr: Option<&str>,
...
pub(in crate::runner::tests) fn write_task_api_env_unreachable_manifest(
    root: &Path,
    task_env_file_expr: Option<&str>,
    step_env_file_expr: Option<&str>,
    env_step_expr: &str,
