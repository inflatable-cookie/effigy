...
pub(crate) fn render_demo_help<R: Renderer>(renderer: &mut R) -> UiResult<()> {
    ...
    render_info_notices(
        renderer,
        &[
            "Use `effigy demo browser` for the first interactive proof browser, `effigy demo list` for direct CLI discovery, `effigy demo inspect <DEMO_ID>` to inspect one record in detail, and `effigy demo run <DEMO_ID>` to record a new normalized attempt before `stop` or `rerun` when lifecycle control exists for that demo. In text mode, interactive and hybrid run-backed demos now attach directly to the live terminal session, and when the runtime supports it they use a PTY-backed transport so terminal-oriented demos can run honestly while still writing runner-owned logs and receipts. `demo inspect` also exposes the active terminal/session handoff for live demo attempts, including transport metadata, recent output snapshots, and explicit input-forwarding capability signaling, while `demo input <DEMO_ID> --text <TEXT>` remains the bounded forwarding surface when the active runtime exposes it. Inside the browser, `←` and `→` switch between the demo list and the detail pane, `Tab` and `Shift+Tab` switch the selected demo between `Overview`, `History`, `Terminal`, and `Artifacts`, `↑` and `↓` act inside the focused panel, `Enter` opens the action sheet from the list or activates the selected detail-side action, retained attempt, terminal refresh, or artifact, `Esc` closes overlays, returns non-overview tabs to `Overview`, or quits from `Overview`, `/` edits search, and `f` opens the single filter sheet for owner/tag/mode/cover/status/gap/stale/grouping controls.",
        ],
    )?;
