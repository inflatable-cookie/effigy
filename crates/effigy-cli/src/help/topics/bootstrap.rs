...
const BOOTSTRAP_HELP: StandardTopicHelpSpec = StandardTopicHelpSpec {
    ...
    notices: text_lines![
        ...
        "Use `--fresh` when you need an isolated throwaway runtime namespace for bootstrap testing. Pair it with `effigy deliver bootstrap teardown` afterward to clean up the session-scoped volumes and runtime.",
    ],
    usage: text_lines![
        ...
        "effigy --json deliver bootstrap <GIT_URL> --plan",
    ],
    ...
    examples: text_lines![
        ...
        "effigy deliver bootstrap https://github.com/inflatable-cookie/loophole.git --path ./loophole --plan",
        "effigy deliver bootstrap git@github.com:inflatable-cookie/loophole.git --branch main --no-start --plan",
        "effigy deliver bootstrap git@github.com:inflatable-cookie/loophole.git --backend docker --fresh --no-start",
        "effigy deliver bootstrap git@github.com:inflatable-cookie/legacy.git --db-seed ./backups/latest.sql --start",
        "effigy deliver bootstrap git@github.com:inflatable-cookie/legacy.git --db-seed app=oci://ghcr.io/acme/private-data:uat --start",
        "effigy deliver bootstrap git@github.com:acowtancy/market.git --db-seed legacy_mysql --start",
        "effigy deliver bootstrap git@github.com:Cumberland-BS/cbs.git --db-seed cbs=./backups/cbs.sql --db-seed cbs-mortcalc=./backups/cbs-mortcalc.sql --start",
        "effigy deliver bootstrap git@github.com:acowtancy/market.git --fresh --no-start",
        "effigy deliver bootstrap children status",
        "effigy deliver bootstrap children sync",
        "effigy deliver bootstrap children sync --fetch-only --json",
