...
const GRAPH_HELP: StandardTopicHelpSpec = StandardTopicHelpSpec {
    ...
    usage: &[
        ...
        "effigy repo graph search [--repo <PATH>] [--limit <N>] <QUERY> [--json]",
        "effigy repo graph files [--repo <PATH>] [--limit <N>] [--json]",
        "effigy repo graph node [--repo <PATH>] <ID> [--json]",
        "effigy repo graph callers [--repo <PATH>] [--limit <N>] <ID> [--json]",
        "effigy repo graph callees [--repo <PATH>] [--limit <N>] <ID> [--json]",
        "effigy repo graph impact [--repo <PATH>] [--limit <N>] <TARGET> [--json]",
        "effigy repo graph affected [--repo <PATH>] [--depth <N>] [--limit <N>] [--stdin] <PATH>... [--json]",
        "effigy repo graph context [--repo <PATH>] [--max-files <N>] [--max-bytes <N>] [--language <ID>]... [--path <PREFIX>]... <REQUEST> [--json]",
        "effigy repo graph explore [--repo <PATH>] [--max-files <N>] [--max-bytes <N>] [--language <ID>]... [--path <PREFIX>]... <REQUEST> [--json]",
    ],
    ...
    examples: &[
        ...
        "effigy repo graph index --json && effigy repo graph status --json",
        "effigy repo graph context \"trace deploy provider export\" --max-files 8 --json",
        "effigy repo graph explore \"trace graph watch implementation\" --max-files 6 --json",
        "effigy repo graph search deploy --limit 20 --json",
        "effigy repo graph impact src/runner/release_command/mod.rs --json",
        "git diff --name-only | effigy repo graph affected --stdin --json",
        "mv .effigy/graph .effigy/graph.backup-$(date +%s) && effigy repo graph index --json",
        "effigy repo graph watch --debounce-ms 1000 --json",
        "effigy repo graph node symbol:rust:run_release --json",
        "effigy repo graph callers symbol:rust:run_release --json",
    ],
};
