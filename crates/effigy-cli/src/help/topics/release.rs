...
const RELEASE_HELP: StandardTopicHelpSpec = StandardTopicHelpSpec {
    ...
    notices: text_lines![
        ...
        "Text-mode release preparation and execution now use compact review menus by default; interactive prepare can jump between version review, mutation inspection, gate results, and final approval, while interactive execute can jump between stale warnings, prepared-state review, working-tree inspection, and final approval. Those menus now keep a compact command legend plus the current selected version or stale-acknowledgement state visible while you review, mark which sections were already reviewed, and blocked prepare/execute output now adds suggested remediation actions instead of only raw blocker lines. `effigy deliver release resume` is the dedicated prepared-state recovery entrypoint: it summarizes `.release-prepared.json`, highlights drift since prepare time, and can hand off directly into execute review. Prepared release state now records source fingerprints, so `resume` and `execute` can detect branch drift, HEAD movement, and prepared-file content drift instead of relying only on raw working-tree presence checks. Those recovery menus now also expose direct `gates`, `reprepare`, and `discard` shortcuts so operators can inspect gates, regenerate prepared state, or clear stale state without leaving the interactive flow. `--plan` stays non-destructive, `--dry-run` aliases that preview mode, and `--yes` stays the explicit non-interactive path.",
    ],
    usage: text_lines![
        "effigy deliver release status [--repo <PATH>] [--check-gates] [--json]",
        "effigy deliver release gates [--repo <PATH>] [--json]",
        "effigy deliver release resume [--repo <PATH>] [--allow-stale] [--json]",
        "effigy deliver release verify-install [--repo <PATH>] [--tag <TAG>] [--repo-url <URL>] [--json]",
        "effigy deliver release validate [--repo <PATH>] [--tag <TAG>] [--json]",
        "effigy deliver release check-binary [--repo <PATH>] <BIN> --glibc-floor <VER> [--json]",
        "effigy deliver release preflight [--repo <PATH>] [--tag <TAG>] [--skip-docs] [--skip-smoke] [--output <PATH>] [--json]",
        "effigy deliver release proof [--repo <PATH>] --tag <TAG> [--crate-version <VER>] [--repo-url <URL>] [--brew-formula <NAME>] [--skip-homebrew] [--artifacts-dir <DIR>] [--json]",
        "effigy deliver release evidence validate [--repo <PATH>] --artifacts-dir <DIR> [--expect-homebrew] [--json]",
        "effigy deliver release evidence closeout [--repo <PATH>] --tag <TAG> --artifacts-dir <DIR> [--output <PATH>] [--owner <NAME>] [--expect-homebrew] [--json]",
        "effigy deliver release evidence summary [--repo <PATH>] --tag <TAG> --artifacts-dir <DIR> [--crate-version <VER>] [--repo-url <URL>] [--brew-formula <NAME>] [--homebrew-executed] [--log-file <NAME>]... [--json]",
        "effigy deliver release simulate [--repo <PATH>] [--version <SEMVER>] [--json]",
        "effigy deliver release prepare [--repo <PATH>] [--check-gates]",
        "effigy deliver release prepare (--plan|--dry-run) [--repo <PATH>] [--check-gates] [--version <SEMVER>] [--json]",
        "effigy deliver release prepare --yes [--repo <PATH>] [--check-gates] [--version <SEMVER>] [--json]",
        "effigy deliver release execute [--repo <PATH>] [--allow-stale]",
        "effigy deliver release execute (--plan|--dry-run) [--repo <PATH>] [--allow-stale] [--json]",
        "effigy deliver release execute --yes [--repo <PATH>] [--allow-stale] [--json]",
        "effigy --json deliver release status [--repo <PATH>] [--check-gates]",
    ],
    ...
    examples: text_lines![
        ...
        "effigy deliver release status --repo /path/to/workspace",
        "effigy deliver release status --check-gates",
        "effigy deliver release gates",
        "effigy deliver release resume",
        "effigy deliver release resume --allow-stale",
        "effigy deliver release verify-install --tag v0.2.5",
        "effigy deliver release validate --tag v0.2.5",
        "effigy deliver release check-binary ./effigy-x86_64-unknown-linux-gnu --glibc-floor 2.35",
        "effigy deliver release preflight --tag v0.2.5 --output ./artifacts/distribution-preflight-v0.2.5.env",
        "effigy deliver release proof --tag v0.2.5 --artifacts-dir ./artifacts/distribution-v0.2.5",
        "effigy deliver release evidence validate --artifacts-dir ./artifacts/distribution-v0.2.5",
        "effigy deliver release evidence closeout --tag v0.2.5 --artifacts-dir ./artifacts/distribution-v0.2.5",
        "effigy deliver release evidence summary --tag v0.2.5 --artifacts-dir ./artifacts/distribution-v0.2.5 --homebrew-executed --log-file 01-tag-install-validation.log",
        "effigy deliver release simulate",
        "effigy deliver release simulate --version 0.2.8",
        "effigy deliver release prepare",
        "effigy deliver release prepare --dry-run --version 0.2.8",
        "effigy deliver release prepare --plan --version 0.2.8",
        "effigy deliver release prepare --yes --check-gates --version 0.2.8",
        "effigy deliver release execute",
        "effigy deliver release execute --allow-stale",
        "effigy deliver release execute --dry-run",
        "effigy deliver release execute --plan",
        "effigy deliver release execute --yes",
        "effigy --json deliver release prepare --plan --check-gates",
        "effigy --json deliver release status --check-gates",
    ],
};
