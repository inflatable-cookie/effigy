...
const CONTAINER_HELP: StandardTopicHelpSpec = StandardTopicHelpSpec {
    ...
    notices: text_lines![
        ...
        "`container profile status` reports Colima profile sizing against Effigy's managed targets, `container profile resize` applies managed sizing in place by stopping and restarting the profile, and `container profile recreate --yes` stays as the destructive last-resort rebuild path.",
