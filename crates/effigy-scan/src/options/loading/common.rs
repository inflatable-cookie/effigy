...
use crate::constants::TASK_MANIFEST_FILE;
use crate::error::ScanError;
use effigy_manifest::{load_task_manifest, LoadedCatalog, TaskManifest};
...
fn build_manifest_backed_options<T>(config: Option<&T::ManifestConfig>) -> Result<T, ScanError>
where
...
where
    FDefault: FnOnce() -> T,
    FBuild: FnOnce(&TaskManifest) -> Result<T, ScanError>,
{
...
fn load_scan_manifest(manifest_path: &Path) -> Result<TaskManifest, ScanError> {
    load_task_manifest(manifest_path).map_err(ScanError::from)
}
