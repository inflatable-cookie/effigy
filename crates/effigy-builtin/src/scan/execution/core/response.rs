...
pub(super) fn fail_on_findings_error(
    ...
    if fail_on_findings && finding_count > 0 {
        return Err(BuiltinError::BuiltinScanNonZero {
            finding_count,
...
) -> Result<RenderedScanOutput, BuiltinError>
where
    TOptions: ScanCommonOptions,
...
fn write_report(output_path: &Option<PathBuf>, rendered_text: &str) -> Result<(), BuiltinError> {
    if let Some(path) = output_path {
