...
pub(crate) enum ParseLoopAction {
    Handled,
    Unknown,
    Break,
}
...
impl<'a> BuiltinArgParser<'a> {
    ...
    pub(super) fn next_value(&mut self, missing_message: &str) -> Result<&'a str, BuiltinError> {
        let value = self
            .args
            .get(self.index)
            .ok_or_else(|| BuiltinError::task_invocation(missing_message))?;
        self.index += 1;
        Ok(value.as_str())
    }
    ...
    pub(crate) fn positional_task_invocation(&self, name: &str) -> TaskInvocation {
        TaskInvocation {
            name: name.to_owned(),
    ...
    pub(crate) fn parse_loop_collect_unknown<F>(
        &mut self,
        mut on_arg: F,
    ) -> Result<Vec<String>, BuiltinError>
    where
        F: FnMut(&mut Self, &str) -> Result<ParseLoopAction, BuiltinError>,
    {
    ...
    pub(crate) fn parse_loop_require_no_unknown<F>(
        &mut self,
        task_name: &str,
        on_arg: F,
    ) -> Result<(), BuiltinError>
    ...
    pub(crate) fn parse_loop_require_no_unknown_with_prefix<F>(
        &mut self,
        task_name: &str,
        prefix: &str,
        on_arg: F,
    ...
    pub(crate) fn unknown_if_flag_or<F>(
        &self,
        arg: &str,
        on_positional: F,
    ) -> Result<ParseLoopAction, BuiltinError>
    where
        F: FnOnce(&str) -> Result<ParseLoopAction, BuiltinError>,
    {
        if arg.starts_with('-') {
