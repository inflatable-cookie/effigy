...
pub(super) fn build_matcher(
    include: &[String],
    exclude: &[String],
) -> Result<WatchMatcher, BuiltinError> {
...
pub(super) fn collect_snapshot(
    root: &Path,
    matcher: &WatchMatcher,
) -> Result<HashMap<PathBuf, FileStamp>, BuiltinError> {
...
fn build_glob_set(patterns: &[String], label: &str) -> Result<GlobSet, BuiltinError> {
    ...
    builder.build().map_err(|error| {
        BuiltinError::task_invocation(format!("failed to compile `{label}` glob set: {error}"))
    })
}
