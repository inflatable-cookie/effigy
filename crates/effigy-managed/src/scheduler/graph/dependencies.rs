...
pub fn build_step_dependencies(
    task_name: &str,
    steps: &[ManifestManagedRunStep],
    id_to_index: &BTreeMap<String, usize>,
) -> Result<Vec<Vec<usize>>, ManagedError> {
    steps
        .iter()
        .enumerate()
...
pub fn build_step_dependents(dependencies: &[Vec<usize>]) -> Vec<Vec<usize>> {
