...
impl WorkflowPhaseHandler for MockWorkflowPhaseHandler {
    fn resolve_root(
        ...
        if self.should_fail(WorkflowPhase::RootResolution) {
            return Err(DoctorError::task_invocation("simulated resolve failure"));
        }
        Ok(ResolvedTarget {
...
fn assert_task_invocation_contains(err: DoctorError, expected: &str, context: &str) {
    match err {
        DoctorError::TaskInvocation(message) => {
            assert!(
