...
pub(crate) enum ManifestParseFinding {
    Read {
...
impl ManifestParseFinding {
    pub(crate) fn read_failure(manifest_path: &Path, error: impl std::fmt::Display) -> Self {
        Self::Read {
    ...
    pub(crate) fn toml_syntax_failure(manifest_path: &Path, error: impl std::fmt::Display) -> Self {
        Self::TomlSyntax {
            manifest_path: manifest_path.to_path_buf(),
            error: error.to_string(),
        }
    ...
    pub(crate) fn strict_parse_failure(
        manifest_path: &Path,
