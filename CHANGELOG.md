...
- The underlay bundle now owns the standard root `health`, `validate`, and `qa` aggregators. Consumers can opt into docs-aware defaults with `bundle.dirs.docs`, and repo roots only need explicit overrides when they diverge from the standard docs/api/client/ui/front/admin shape.
...
- Internal: split `effigy-containers/src/tests.rs` (~2.1 kLOC, 49 tests) into themed children under `tests/`: `policies.rs` (policy loading + runtime validation), `compose.rs` (generated compose synthesis, ports, DNS, mounts), and `volumes_reports.rs` (workspace ownership, ejection, status/stats/data reports). The parent `tests.rs` now holds shared imports and the `temp_repo` / `with_temp_effigy_home` helpers. Test surface is unchanged; clears the file from the doctor god-files scan.
- Internal: split `effigy-rhai/src/lib.rs` (~1.7 kLOC) by moving the ~840-line `register_host_api` function (all `engine.register_fn(...)` registrations) into a sibling `host_api.rs` module. The crate's public API (`execute_rhai_script`, `ScriptContext`, `HostCallbacks`, `EffigyCommandError`, `HostCommandOutput`, `RhaiHostError`, env constants, helpers) is unchanged; clears the file from the doctor god-files scan.
- Internal: split `effigy-containers/src/workspace.rs` (~1.6 kLOC) by moving its two `#[cfg(test)]` test modules (`library_mount_tests` + `host_git_mount_tests`, 22 tests total) into a sibling `workspace_tests.rs` file declared via `#[path = "workspace_tests.rs"] mod tests;`. Clears the file from the doctor god-files scan.
- Internal: split `effigy/src/runner/system_command/workspace.rs` (~1.8 kLOC) by extracting its `#[cfg(test)] mod tests` block (~25 tests covering linux artifact resolution, repo discovery, permission rendering, handoff/shutdown ownership) into a sibling `workspace_tests.rs` file declared via `#[path = "workspace_tests.rs"] mod tests;`. Clears the file from the doctor god-files scan.
- Internal: split `effigy/src/runner/container_command/gateway_registration.rs` (~1.7 kLOC) by extracting its `#[cfg(test)] mod tests` block (26 tests covering route resolution, runtime validation, host-port selection, loopback registry pruning, route-table I/O) into a sibling `gateway_registration_tests.rs` file declared via `#[path = "gateway_registration_tests.rs"] mod tests;`. Final god-file in the doctor scan; the project now reports `summary ok:16 warn:0 err:0`.
- Internal: `EnvGuard` (test helper for process-env mutation) now acquires the shared test serialization lock for its lifetime, eliminating env-var races between parallel tests. Backed by a small reentrant lock built on stable primitives so existing call sites that already hold `lock_test()` continue to work without deadlocking. Six `workspace_tests.rs` tests that hand-rolled `unsafe { set_var("HOME", ...) }` save/restore are converted to `EnvGuard::set_many(&[("HOME", Some(...))])` so they share the same serialization (and become panic-safe via Drop).
